...
qiao.bs.modalstr = function(opt){
	...
	var foot = '';
	if(opt.foot){
		foot += '<div class="modal-footer"><button type="button" class="mybtn-primary bsok">'+opt.okbtn+'</button>';
		if(opt.btn){
			foot += '<button type="button" class="mybtn bscancel ml15">'+opt.qubtn+'</button>';
		}
		foot += '</div>';
	}
