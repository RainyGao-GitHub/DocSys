$(function(){
	...
	$("#floatDiv").hide();
//	showBusiSmlType();
	markFormTextAreaFmt("editorForm");
	...
	if(prjArea!=null&&prjArea!=undefined){
		setPCA(prjArea.substring(0,2)+"0000",prjArea.substring(0,4)+"00",prjArea);
	}else{
		queryProvince();
	}
//	queryProvince();
	...
	//向后台提交数据时格式化textArea
	var ta = $($("#editorForm").find("textArea"))[0];
	var taVal = $(ta).val();
	var a = taVal.split("<br>").join("\n");
	$(ta).val(a);
	createDialog("bstDialog","添加项目类型","bst_diaDiv");
	hideDialogFooter("bstDialog");
	
	//======如果是更新操作，要初始化下拉框。
	var option = $("#optType").val();
	if(option=="update"){
		
	}
	
	findMyLastThreeProject();
	showMyIntro();
...
var imgdivnum = 1;
function createaImgDiv(dom){
	imgdivnum++;
	var imgDivHtml = "<div>"
					+"	<div id='imgDiv' class='imgDiv' style='filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod=scale);'></div>"
					+"	<div class='thinBorder fLeft m5' style='width: 300px;height: 300px;overflow: hidden;' >"
					+"		<img id='imgPreView' onclick='path"+imgdivnum+".click()' class='imgWH300px imgPreView' src='webPages/images/add.png' alt='点击选择上传图片' title='点击选择上传图片'/>"
					...
					+"			<textarea style='width: 100%;height: 170px;resize: none;' maxlength='500px'></textarea>"
					+"		</div>"
					+"		<div class='clearfix'></div>"
					+"	</div>"
					+"	<input type='file' id='path"+imgdivnum+"' style='display:none' onchange='setImage(this)'>"
					+"</div>";
	$(dom).before(imgDivHtml);
}


function saveProject(dom){
	unableDbClick();
	if(!formAjaxSubmitCheck("editorForm")){
		unableDbClick();
		return;
	}else if(!$("#planId").val()){
		//支付计划不能为空
		bootstrapQ.alert("支付计划未设置！");
		unableDbClick();
		return;
	}
...
function queryProgramerNeedData(){
	var projectId = $("#projectId").val();
	var callback = function(data){
		createTable(data);
	};
	$.post("queryProgramerNeedData.do",{'projectId':projectId},callback,"json");
}
...
function createTable(data){
	...
	for(var i = 0;i<data.length;i++){
		...
		var tmp = "<tr onclick='setCbChecked(this)'>"
			+"	<td><input type='checkbox' onmouseover='setCFlagFalse()' onmouseout='setCFlagTrue()' value='" + d.id + "' /></td>"
			+"	<td>" + getJob(d.jposition).main_job + "/" + getJob(d.jposition).sub_job + "</td>"
			+"	<td>"+ year[d.years] +"</td>"
			+"	<td>"+edu[d.eduLevel]+"</td>"
			+"	<td><a href='' data-toggle='modal' data-target='#myModal1' onclick='changeDialogUrl(\"detail\","+d.id+");'>详细</a></td>"
			+"</tr>"
		$("#pnTable").append(tmp);
	}
}
...
var cFlag = true;
function setCbChecked(dom){
	if(cFlag){
		var cb = $(dom).find("[type=checkbox]")[0];
		if(cb.checked){
			cb.checked=false;
		}else{
			cb.checked=true;
...
function setCFlagFalse(){
	cFlag = false;
}

function setCFlagTrue(){
	cFlag = true;
}

function checkAllCb(dom){
	var cb = $(dom)[0];
	var cbs = $("#pnTable").find("[type=checkbox]");
	if(cb.checked){
 ...
 * @param option
 */
function changeDialogUrl(option,id){
	
	var pnForm = document.getElementById("pnForm");
	pnForm.action = "programerNeed.do?option=" + option;
	if(option=='add'){
		showDialogFooter("myModal1");
	}else if(option=='update'){
		//查找被选中的行
		var trs = $("#pnTable").find("tr:gt(1)");
		var cc = 0; //统计被
		var c = $(trs).find("[type=checkbox]");
		var _cb;
		...
		if(cc==0){
			...
			$.post("programerNeed.do?option=delete",{'id':cc.toString()},callback,"json");
			queryProgramerNeedData();
		}
...
function initPnDetail(id){
	...
	var callback = function(data){
		$("#pnid").val(data.id);
		index = getJob(data.jposition).pId;
		setCbSelect($("#cJob"),index);
		setCbSelect($("#cJob2"),data.jposition);
		$("#years").val(data.years);
		$("#eduLevel").val(data.eduLevel);
		var tmp = data.othersNeed;
		console.log("替换前:"+tmp);
		tmp = tmp.replace(/<br>/g,"\n").replace(/&nbsp;/g," ");
		console.log("替换后:"+tmp);
		$("#othersNeed").val(tmp);
	}
	$.post("queryPnDetail.do",{"id":id},callback,"json");
}
 ...
 */
function ajaxSubmit(){
	//将textarea中的回车以及空格替换成html格式
	var ta = $("#othersNeed").val();
	ta = ta.replace(/[\n\r]/g,"<br>").replace(/[ ]/g,"<nbsp/>"); 
	$("#othersNeed").val(ta);

	console.log("ajax发送表单！");
	//获取表单中的参数并将其整理为json格式
	var pnForm = document.getElementById("pnForm");
	var param = $('#pnForm').serialize();
	param = param.replace(/\+/g," "); 
	//由于serialize方法会调用encodeURIComponent()将中文转码。下面调用decode保持正常格式。防止乱码
	param = decodeURIComponent(param,true); 
    var jparam = changeUrlParamsToJson(param);
    jparam.projectId = $("#projectId").val(); //记录的id
    ...
    var callback = function(data){
    	...
    	alert(data.obj);
    	queryProgramerNeedData();
    	return false;//ajax提交后不能再用普通方式再次提交
    }
    
    var url = document.getElementById("pnForm").action;
...
function findMyLastThreeProject(){
	var callback = function(data){
		...
		$.each(data.obj,function(i,item){
			...
			var str = "<div class='p5 thinBorder margin10_0'>"
				...
				+"			<img width='100%' src='"+ $("#basePath").val()+ "webPages/upload/project/" + d.PROLOG +"' onerror='this.src=\"webPages/images/project.jpg\"'/>"
				+"		</a>"
				+"	</div>"
				+"	<div>"
				+"		<h5>"
				+"			<p><a href='toProject2.do?id="+d.ID+"' target='_blank'>"+d.NAME+"</a></p>"
				+"		</h5>"
				+"	</div>"
				+"</div>";
	...
	$.post("queryMyLastThreeProject.do",null,callback,"json");
}

/**
 * 老版，已废弃 2016-5-24 22:04:49 added comment
 */
