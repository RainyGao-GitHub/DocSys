...
<html>
	...
	<body>
					...
					<div class="col-sm-9">
						...
						<div>
							<h3 class="sectionTitleA bold c56 f18 mb20" style="width:100%" id="panel_product_media">
								<c:if test="${empty pro.id}">发布项目</c:if>
								<c:if test="${not empty pro.id}">修改项目</c:if>
							</h3>
							<form id="editorForm" action="addProject.do" method="post" enctype="multipart/form-data">
								<c:if test="${empty pro.id}">
									<input type="hidden" id="optType" name="optType" value="add"/>
									<input type="hidden" id="projectId" name="id" value="${projectId}"/>
								</c:if>
								<c:if test="${not empty pro.id}">
									<input type="hidden" id="optType" name="optType" value="update"/>
									<input type="hidden" id="projectId" name="id" value="${pro.id}"/>
								</c:if>
								...
								<table width="100%" class="form-table">
									<tr>
										...
										<td width="62%">
											<div class="form-group has-feedback p0m0">
												<input id="name" name="name" needvalicate="true" valicate="_required _maxlen=20" maxlength="20" type="text" class="form-control" value="${pro.name}"/>
											</div>
										</td>
										<td width="20%"></td>
									</tr>
									<tr>
										...
										<td>
											<div class="form-group has-feedback p0m0">
												<textarea id="intro" name="intro" class="form-control" needvalicate="true" valicate="_required _maxlen=500" style="width: 100%;height: 170px;resize: none;" maxlength="500">${pro.intro}</textarea>
											</div>
										</td>
										<td>
											<!-- <input type="checkbox" name="prjOutSource" value="1" checked="checked"/>允许工作室或者企业外包项目 -->
										</td>
									</tr>
									<tr>
										<td>海报</td>
										<td>
											<input type="file" id="proImg" name="proImg" style="padding: 0px;" class="form-control" value="${pro.proLog}"/>
										</td>
										<!--
										<td>将在页面的顶部展示大图片</td>  
										-->
									...
									<%-- <tr>
										<td>项目目前阶段</td>
										<td>
											<select id="stage" name="stage" class="form-control" value="${pro.stage}">
												<option value="1">正在筹划</option>
									...
									<tr>
										...
										<td id="pca" class="pca" class="align-left">
											...
											<select id="area" name="prjArea" style="width: 32%;">
												<option value="-1">请选择</option>
											</select>
											<input type="hidden" id="prjArea" value="${pro.prjArea}"/>
										</td>
										<td></td>
									</tr>
									<tr>
										...
										<td>
											...
											<div class="form-group has-feedback p0m0 fLeft" style="width:40%">
												<input type="text" id="startPrice" maxlength="8" needvalicate="true" valicate="_n _maxlen=8 _lt=endPrice" placeholder="开始价格" class="form-control " name="startPrice" value="${pro.startPrice}" />
											</div>
											<span class="p10 fLeft" style="width:10%;text-align: center">至</span>
											<div class="form-group has-feedback p0m0 fLeft" style="width:40%">
												<input type="text" id="endPrice" maxlength="8" needvalicate="true" valicate="_n _maxlen=8 _gt=startPrice" placeholder="结束价格" class="form-control" name="endPrice" value="${pro.endPrice}"/>
											</div>
											<div class="clear"></div>
										</td>
										<td>
									...
									<%-- <tr>
										<td>选择行业</td>
										<td>
											<select id="busiType" style="width:50%" onchange="setBusiType2(this)" class="form-control fLeft" value="${pro.busiType }"></select>
											<select id="busiType2" style="width:50%" name="busiType" class="form-control fLeft" value=""></select>
											<div class="clear"></div>
										</td>
									...
									<tr>
										...
										<td>
											...
											<div class="clear"></div> --%>
											<select id="busiSmlType" name="busiSmlType"  class="form-control fRight" value="${pro.busiSmlType }">
										...
										<td>
											<a onclick="showAddBstDia();" class="mybtn-primary"><i class="glyphicon glyphicon-plus"></i>添加小类</a>
										</td>
									</tr>
									-->
									<tr>
										<td>标签</td>
										<td>
											<input type="text" id="keyWord" name="keyWord" placeholder="最多添加五个标签" readonly="readonly" class="form-control" onclick="showTypeLabel()" value="${pro.keyWord }"/>
											<input type="hidden" id="keyWord_ids" name="keyWord_ids" value="${pro.keyWord_ids}"/>
									...
									<tr>
										...
										<td style="text-align: left;max-width: 300px;;padding: 20px;overflow-x: auto;overflow-y: hidden;">
											...
											<input type="hidden" name="planId" id="planId" value="${pro.planId}"/>
									...
									<%-- <tr>
										<td>关键词</td>
										<td>
											<select id="keyword_selecter" value="${pro.keyWord}" name="keyWords" class="js-example-basic-multiple form-control" multiple="multiple">
											</select>
										</td>
									</tr> --%>
									<!-- <tr>
										<td>需要人才</td>
										<td colspan="2">
											<span class="redStar">注意：操作后点击最下方保存才会保存数据</span>
											<table id="pnTable" class="table content-table pinfo table-striped table-bordered">
							  					<tr>
							  						<th colspan="5" class="align-right">
							  							<a href="" data-toggle="modal" data-target="#myModal1" onclick="changeDialogUrl('add');" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-plus"></span>&nbsp;添加</a>
												  		<a href="javaScript:void(0)" onclick="changeDialogUrl('update');" class="btn btn-default  btn-sm" ><span class="glyphicon glyphicon-pencil"></span>&nbsp;修改</a>
												  		<a href="javaScript:void(0)" onclick="changeDialogUrl('delete');" class="btn btn-default  btn-sm"><span class="glyphicon glyphicon-minus"></span>&nbsp;删除</a>
							  						</th>
							  					</tr>
									  			<tr class="text-center th" >
									  				<td><input type="checkbox" onchange="checkAllCb(this);" /></td>
									  				<td>要求职业</td>
									  				<td>工作年限</td>
									  				<td>学历要求</td>
									  				<td>查看详细</td>
									  			...
									  			<tr>
									  				...
									  				<td>2年</td>
									  				<td>本科</td>
									  				<td><a href="" data-toggle="modal" data-target="#myModal1" onclick="changeDialogUrl('detail');">详细</a></td>
									  			</tr>
									  		</table>
										</td>
									</tr> -->
									...
									<tr>
										...
										<td class="align-left" colspan="2">
											...
											<!-- 加载编辑器的容器 -->
											<script id="container" name="content" type="text/plain" style="width: 100%;">${pro.content}</script>
											
										</td>
									</tr>
									<tr>
										<td colspan="3">
											<a class="mybtn-primary" href="javascript:void(0);" onclick="saveProject(this);">保存</a>
										</td>
									</tr>
								</table>
							</form>
		...
		<script type="text/javascript" src="<%=basePath %>webPages/pages/tianTianTou/pageJs/newproject.js"></script>
	</body>
</html>
