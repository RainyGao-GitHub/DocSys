...
//文件打印接口  
//async/await 方案
//async function startPrint(pdfUrl) 
//{  
...
//链式方案
//// PDF.js 全局配置  
//pdfjsLib.GlobalWorkerOptions.workerSrc = '/DocSystem/web/static/pdfjs/build/pdf.worker.js';
//
//// 主打印函数  
//function startLegacyPrint(pdfUrl) {  
...
//新窗口打印方案： 方案不好
//pdfjsLib.GlobalWorkerOptions.workerSrc = '/DocSystem/web/static/pdfjs/build/pdf.worker.js';
//function startWindowPrint(pdfUrl) {  
//    // 创建独立打印窗口  
...
//          renderTasks.push(  
//            pdf.getPage(i).then(function(page) {  
//              return renderPage(page, printWindow);  
//            })  
//          );  
//        }  
...
//  }  
//
//  function renderPage(page, targetWindow) {  
//    const viewport = page.getViewport({ scale: 2 });  
...
//      viewport: viewport  
//    }).promise.then(function() {  
//      targetWindow.document.getElementById('container').appendChild(canvas);  
//    });  
//  }  
pdfjsLib.GlobalWorkerOptions.workerSrc = '/DocSystem/web/static/pdfjs/build/pdf.worker.js';
function startSafePrint(pdfUrl) {  
    // 创建隔离容器  
    const safeContainer = createSandboxContainer();  
    const temporaryStyles = [];

    // 初始化打印环境  
    initPrintEnvironment(safeContainer, temporaryStyles)  
      .then(() => loadAndRenderPDF(pdfUrl, safeContainer))  
      .then(() => executePrint(safeContainer))  
      .catch(handlePrintError)  
      .finally(() => cleanResources(safeContainer, temporaryStyles));  
  }  
  ...
  // 初始化打印样式  
  function initPrintEnvironment(container, temporaryStyles) {  
    return new Promise(resolve => {  
      // 动态创建打印专用样式表  
      const style = document.createElement('style');  
      style.textContent = `  
        @media print {  
          body > :not(${container.tagName}) {  
  ...
  // PDF加载与渲染  
  function loadAndRenderPDF(url, container) {  
    return pdfjsLib.getDocument(url).promise  
      ...
      .then(pages => {  
        return Promise.all(pages.map(page => renderPage(page, container)));  
      });  
  }  
  ...
  // 执行打印  
  function executePrint(container) {  
    return new Promise(resolve => {  
      // 触发打印前准备  
  ...
  // 资源清理  
  function cleanResources(container, styles) {  
    container.remove();  
    styles.forEach(style => style.remove());  
  }  
function printDoc(treeNodes)
{
	console.log("printDoc treeNodes:", treeNodes);
	if(!treeNodes || treeNodes == null)
	{
		showErrorMessage({
    		id: "idAlertDialog",	
    		title: _Lang("提示"),
    		okbtn: _Lang("确定"),
    		msg: _Lang("请选择需要打印的文件") + "!",
    	});
		return;
	}
	
	getDocFileLink(treeNodes[0], startSafePrint, showErrorMessage, "REST"); //要求获取RESTFUL风格的fileLink
}
