...
{	
	...
	private static Document buildDocument(Doc doc, String content) {
        ...
        document.add(new TextField("vid", doc.getVid()+"", Store.YES));
        document.add(new TextField("pid", doc.getPid()+"", Store.YES));	//docId总是可以通过docPath 和 docName计算出来
        document.add(new TextField("docId", doc.getDocId()+"", Store.YES));	//docId总是可以通过docPath 和 docName计算出来
